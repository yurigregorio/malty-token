...
const copy = {
  en: {
    ...
    intro: "Built transparently on Solana. MALTY brings community, a recognizable identity and a long-term vision for positive animal impact together.",
    explore: "Discover MALTY",
    verify: "Verify token",
    storyEyebrow: "WHY MALTY",
    storyTitle: "One name. One identity.",
    storyText: "MALTY is the project, the token and the brand. It is being built around four simple pillars: transparency, a recognizable identity, an open community and a future initiative to support animal welfare.",
    ...
    givesText: "MALTY Gives is a planned initiative to extend the MALTY mission into positive impact for animals. Before any contribution program starts, the project intends to publish its policy, selection criteria and how contributions will be evidenced.",
  ...
  pt: {
    ...
    intro: "Construído com transparência na Solana. MALTY une comunidade, uma identidade própria e uma visão de longo prazo para gerar impacto positivo para animais.",
    explore: "Conhecer MALTY",
    verify: "Verificar token",
    storyEyebrow: "POR QUE MALTY",
    storyTitle: "Um nome. Uma identidade.",
    storyText: "MALTY é o projeto, o token e a marca. Ele está sendo construído sobre quatro pilares simples: transparência, identidade reconhecível, comunidade aberta e uma futura iniciativa de apoio ao bem-estar animal.",
    ...
    givesText: "MALTY Gives é uma iniciativa planejada para ampliar a missão MALTY e gerar impacto positivo para animais. Antes de qualquer programa de contribuição começar, o projeto pretende publicar sua política, critérios de seleção e como cada contribuição será comprovada.",
...
export default function Home() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#080a0d] text-[#f7f1e5] selection:bg-[#d9a53d] selection:text-black">
    ...
    <section id="gives" className="border-b border-black/10 bg-[#f2ecdf] text-[#17130d]"><div className="mx-auto grid max-w-6xl gap-8 px-5 py-12 sm:px-8 lg:grid-cols-2 lg:items-center"><div><div className="inline-flex rounded-full bg-[#9a6517]/10 px-3 py-1 text-[10px] font-black tracking-[.18em] text-[#9a6517]">🐾 MALTY GIVES · {t.givesStatus}</div><h2 className="mt-4 text-3xl font-black sm:text-4xl">{t.givesTitle}</h2><p className="mt-4 text-sm leading-6 text-black/60">{t.givesText}</p><p className="mt-4 rounded-xl border border-[#9a6517]/15 bg-white/50 p-3 text-xs text-black/50">{t.givesNote}</p><a href="/gives" className="mt-5 inline-flex text-sm font-bold text-[#9a6517]">MALTY Gives policy →</a></div><div className="grid grid-cols-3 gap-2"><Impact n="01" title={language==="pt"?"Construir":"Build"}/><Impact n="02" title={language==="pt"?"Apoiar":"Support"}/><Impact n="03" title={language==="pt"?"Comprovar":"Prove"}/></div></div></section>
    <section id="roadmap" className="border-b border-white/[.07]"><div className="mx-auto max-w-6xl px-5 py-11 sm:px-8"><Eyebrow>ROADMAP</Eyebrow><h2 className="mt-2 text-3xl font-black">{t.roadmapTitle}</h2><div className="mt-6 grid gap-2 md:grid-cols-4"><Road state="DONE" title="Foundation"/><Road state="NEXT" title="MALTY Community"/><Road state="PLANNED" title="MALTY Gives"/><Road state="FUTURE" title="MALTY Ecosystem"/></div><a href="/updates" className="mt-5 inline-flex text-sm font-bold text-[#e9b949]">Project updates →</a></div></section>
    <section id="faq" className="bg-[#0c0f13]"><div className="mx-auto grid max-w-6xl gap-8 px-5 py-11 sm:px-8 lg:grid-cols-[.7fr_1.3fr]"><div><Eyebrow>FAQ</Eyebrow><h2 className="mt-2 text-3xl font-black">{t.faqTitle}</h2><div className="mt-5 flex flex-col items-start gap-2"><a href="/transparency" className="text-sm font-bold text-[#e9b949]">{t.transparency} →</a><a href="/docs" className="text-sm font-bold text-white/55 hover:text-[#e9b949]">Public documentation →</a></div></div><div className="space-y-2"><Faq q={language==="pt"?"O que é MALTY?":"What is MALTY?"} a={language==="pt"?"MALTY é o nome do projeto, da marca e do token comunitário construído na Solana.":"MALTY is the name of the project, brand and community token built on Solana."}/><Faq q={language==="pt"?"Podem criar mais MALTY?":"Can more MALTY be minted?"} a={language==="pt"?"A Mint Authority foi revogada.":"The Mint Authority has been revoked."}/><Faq q="MALTY Gives" a={language==="pt"?"Iniciativa planejada de apoio ao bem-estar animal. Ainda não existe programa ativo nem percentual definido.":"A planned animal-welfare initiative. There is no active program or defined percentage yet."}/></div></div></section>
