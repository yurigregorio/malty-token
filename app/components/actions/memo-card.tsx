...
export function MemoCard() {
  ...
  const handleMemo = async () => {
    if (cluster === "mainnet") {
      toast.error("Memo transactions are disabled on Mainnet in this application");
      return;
    }
  ...
  return (
    <div className="rounded-2xl border border-border-low bg-card p-6">
      ...
      <p className="mt-1 text-xs text-muted">
        Attach an on-chain note on non-Mainnet development networks with the SPL
        Memo program via the @solana-program/memo kit plugin.
      </p>
      <div className="mt-4 space-y-3">
        ...
        <button
          onClick={handleMemo}
          disabled={isSending || cluster === "mainnet" || memoError != null}
